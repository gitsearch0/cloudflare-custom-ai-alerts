...
const analyzeWithAI = async (events, env) => {
 ...
 Your task is to:
- **Aggregate and analyze** similar events (based on IP, JA4, ASN, path, rule descriptions, bot detection tags).
- **Identify attack types** (e.g., reconnaissance, brute force, vulnerability scan, bot traffic, etc.).
- **Assess severity** based on volume (count), action taken, sensitivity of path, and bot detection tags.
- **Recognize patterns** (e.g., repeated access to .env, low bot score, known malicious ASN, suspicious bot behavior, etc.).
- **Highlight key indicators** from structured fields.

Format exactly like this:

**Attack Summary:**
Brief 3-4 line summary of the attack type, target, and scale.
...
• Bot Detection: [Bot detection tags and patterns]
• Triggered Rules: [List of security rule names/descriptions]

**Recommended Actions:**
1. Immediate: [3-4 critical steps]
2. Mitigation: [3-4 key strategies]
...
const sendAlert = async (events, env) => {
    ...
    const botTags = new Set(events.flatMap(e => e.dimensions.botDetectionTags || []));
...
• Bot Detection Tags: ${Array.from(botTags).join(', ')}
	...
	async scheduled(event, env, ctx) {
		...
		try {
			...
			if (shouldTriggerAlert) {
				...
				await sendAlert(currentEvents, env);
			} else {
